...
async def send(ctx, addy, value):
    try:
        ...
        await ctx.send(
            content=(
                f"<:spider_arrow2:1274756933672501279> **Successfully Sent {value}$**\n"
                f"<:spider_arrow2:1274756933672501279> **From** {ltc_addy}\n"
                f"<:spider_arrow2:1274756933672501279> **To** {addy}\n"
                f"<:spider_arrow2:1274756933672501279> **Transaction Id**: [Click here](https://live.blockcypher.com/ltc/tx/{tx_id})"
            )
        )
        ...
        await ctx.send(
            content=(
                f"<:spider_arrow2:1274756933672501279> **Failed to send LTC Because**: Network error occurred: {e}"
            )
        )
    except KeyError:
        await ctx.send(
            content=(
                f"<:spider_arrow2:1274756933672501279> **Failed to send LTC Because**: Unexpected API response."
            )
        )
    except Exception as e:
        await ctx.send(
            content=(
                f"<:spider_arrow2:1274756933672501279> **An error occurred**: {e}"
            )
        )
